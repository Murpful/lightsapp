...
async function fetchInto(dest, { owner, repo, branch }) {
  ...
  for (const f of readTar(tar)) {
    ...
    // Compare with the separator appended. A bare startsWith(dest) also accepts
    // a SIBLING whose name merely begins with it -- "versions/.staging-x" slips
    // past a "versions/.staging" prefix check, lands outside the staging area,
    // and is not removed by the cleanup that only knows about staging itself.
...
export async function applyUpdate({ owner, repo, branch }) {
  ...
  //
  // Directories are cleared before copying rather than copied over: cp() MERGES
  // into an existing folder, so a file dropped in a later release would linger
  // on the booth for ever, and a half-removed feature is worse than either
  // version of it. The download was verified above, so this cannot empty a
  // folder and then fail to refill it.
  const applied = [];
  for (const item of MANAGED) {
    const from = path.join(staging, item);
    if (!(await exists(from))) continue;
    const to = path.join(ROOT, item);
    if ((await stat(from)).isDirectory()) await rm(to, { recursive: true, force: true });
...
export async function undoUpdate(name) {
  ...
  for (const item of MANAGED) {
    ...
    if (!(await exists(from))) continue;
    // Same reason as applying: merging would leave files from the version being
    // rolled back OUT of, which is not the version anyone asked to go back to.
    const to = path.join(ROOT, item);
    if ((await stat(from)).isDirectory()) await rm(to, { recursive: true, force: true });
    await cp(from, to, { recursive: true, force: true });
    restored.push(item);
